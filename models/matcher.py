...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets, positive_map):
        ...
        # positive_map: [num_all_box, 256] @qu, especially, here num_all_box == batch_size
        # cost_class: [batch_size * num_queries, 1, num_classes] * [1, num_all_box, num_classes] @qu
        # => [batch_size * num_queries, num_all_box] @qu
        cost_class = -(out_prob.unsqueeze(1) * positive_map.unsqueeze(0)).sum(-1)

        # Compute the L1 cost between boxes
        cost_bbox = torch.cdist(out_bbox, tgt_bbox, p=1) # [batch_size * num_queries, num_all_box] @qu
        ...
        C = C.view(bs, num_queries, -1).cpu() # [batch_size, num_queries, num_all_box] @qu
