...
from evaluation import eval_acc, eval_rec, evaluate_alignment
...
def parse_args():
    ...
    parser.add_argument(
        "--prompts_path_val", 
        type=str, 
        default='../dataset/coco/annotations_processed_v1/five_category.json'
    )
    # Criterion
    parser.add_argument(
        "--set_loss", 
...
def main():
    ...
    mseLoss = MSELoss(noise_scheduler, args.snr_gamma)
    ...
    diffqformer, criterion, loss_weight_dict = build_model_combined(args, unet)
    contrastive_loss = ContrastiveLossClass()
    if args.qformer_ckpt is not None:
        diffqformer.load_state_dict_qformer(args.qformer_ckpt, device=accelerator.device)
        if accelerator.is_local_main_process:
            logger.info('Load Qformer ckpt from: ' + args.qformer_ckpt)
    else:
        print('***** Warning: Not load Qformer ckpt! *****')
    
    if args.only_train_lora:
        diffqformer.requires_grad_(False)
    diffqformer.to(accelerator.device)
    ...
    if args.enable_xformers_memory_efficient_attention:
        if is_xformers_available():
            ...
            diffqformer.unet.enable_xformers_memory_efficient_attention()
        else:
    ...
    if accelerator.is_main_process:
        n_parameters = sum(p.numel() for p in diffqformer.parameters() if p.requires_grad)
    ...
    val_dataloader2 = torch.utils.data.DataLoader(val_dataset2, batch_size=args.val_batch_size, shuffle=False, 
                                                    num_workers=args.dataloader_num_workers // 2)
    val_dataset_align = get_dataset('LayoutLLM-T2I', root_dir=None, args=args)
    val_dataloader_align = torch.utils.data.DataLoader(val_dataset_align, batch_size=32, num_workers=args.dataloader_num_workers, 
                                                        shuffle=False, pin_memory=False)
    val_dataloader, val_dataloader2, val_dataloader_align = accelerator.prepare(val_dataloader, val_dataloader2, val_dataloader_align)
    ...
    if args.reset_optimizer:
        diffqformer, criterion, contrastive_loss, train_dataloader = accelerator.prepare(
            diffqformer, criterion, contrastive_loss, train_dataloader)
    else:
        diffqformer, criterion, contrastive_loss, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
        diffqformer, criterion, contrastive_loss, optimizer, train_dataloader, lr_scheduler)
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("DiffQFormer", config=vars(args), 
    ...
    if args.resume_from_checkpoint:
        ...
        if path is None:
            ...
            accelerator.print(global_step, args.gradient_accumulation_steps, resume_global_step)
            accelerator.print(num_update_steps_per_epoch, first_epoch)
            accelerator.print(f'resume_step = {resume_step}' + 
                                f', start training when progress bar reachs {resume_step // args.gradient_accumulation_steps} step.')
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        losses_to_show = ['loss_ce', 'loss_bbox', 'loss_giou', 'cardinality_error', 'loss_contrastive_align', 
                            'loss_contrastive_i2t', 'loss_contrastive_t2i', 'loss_mse', 'loss']
        ...
        for step, batch_dict in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(diffqformer):
                ...
                # forward local positive
                outputs = diffqformer(noisy_latents, timesteps, encoder_hidden_states[0], text_for_object, return_unet_feat=args.unet_feature) 
                # forward global positive
                dict_pos = diffqformer(noisy_latents, timesteps, encoder_hidden_states_whole_img[0], only_matching=True, return_unet_feat='all', return_dict=True, 
                                            detach_unet_feat=args.detach_unet_feature) 
                q_emb_i_t, logit_scale = dict_pos['q_emb_matching'], dict_pos['logit_scale']
                # forward global negative texts
                q_emb_i_tneg, _ = diffqformer(noisy_latents, timesteps, encoder_hidden_states_neg[0], only_matching=True, return_unet_feat=args.unet_feature)
                q_emb_i_trand, _ = diffqformer(noisy_latents, timesteps, encoder_hidden_states_rand[0], only_matching=True, return_unet_feat=args.unet_feature)
                # forward global negative images
                ...
                # shape = (layer, batch, n_query, dim)
                q_emb_ineg_t, _ = diffqformer(noisy_latents_neg, timesteps, encoder_hidden_states_whole_img[0], only_matching=True, return_unet_feat=args.unet_feature)
                q_emb_irand_t, _ = diffqformer(noisy_latents_rand, timesteps, encoder_hidden_states_whole_img[0], only_matching=True, return_unet_feat=args.unet_feature)
                ...
                # denoising
                loss_mse = mseLoss(dict_pos['noise_pred'], target_noise, timesteps)
                loss_dict = {'loss_contrastive_i2t': loss_contrast_i2t, 'loss_contrastive_t2i': loss_contrast_t2i, 
                                'loss_mse': loss_mse}
                loss_weight_dict['loss_mse'] = args.mse_loss_coef
                ...
                if accelerator.sync_gradients:
                    params_to_clip = diffqformer.parameters()
            ...
            if accelerator.sync_gradients:
                ...
                if (global_step % args.checkpointing_steps == 0 or global_step in [10, 20, 50, 100, 150, 200, 300, 400, 500, 600, 700, 800]):
                    end_of_train_dataloader = accelerator.gradient_state.end_of_dataloader
                    save_path = os.path.join(args.output_dir, f"checkpoint-{global_step}")
                    is_debug = 'debug' in args.run_name
                    if accelerator.is_main_process and not is_debug:
                        ...
                        if args.validation_prompts is not None and global_step % args.checkpointing_steps == 0:
                            ...
                            f"{all_prompts_str}")
                            generate(args, accelerator, unet, epoch, weight_dtype, use_lora=True)
                    # evaluation
                    diffqformer.eval()
                    # - alignment
                    sims_align = evaluate_alignment(args, unet, val_dataloader_align, save_path, device, weight_dtype, 
                                                        is_save=(not is_debug), use_lora=True, num_inference_steps=20, 
                                                        show_tqdm=accelerator.is_main_process) # alignment
                    sims_align = accelerator.gather(sims_align)
                    models = (diffqformer, vae, noise_scheduler, tokenizer, text_encoder)
                    # - rec
                    test_stats = eval_rec(args, models, criterion, val_tuples, loss_weight_dict, device, weight_dtype)
                    # - matching
                    models = (diffqformer, vae, text_encoder, tokenizer)
                    txt_acc, img_acc, txt_max_more_than_onces, img_max_more_than_onces = eval_acc(args, val_dataloader, val_dataloader2, 
                                                                        models, noise_scheduler, encoder_hidden_states_uncond, score_batch_func=score_batch_func)
                    ...
                    if accelerator.is_main_process:
                        ...
                        test_stats['clip_sim'] = sims_align.mean().item()
